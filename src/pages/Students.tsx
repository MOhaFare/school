...
import { Search, Plus, Mail, Phone, Eye, Edit, Trash2, Download, Filter, Sun, Moon, Key, Building2 } from 'lucide-react';
...
const transformStudentToCamelCase = (dbStudent: any): Student => ({
  ...
  school_name: dbStudent.schools?.name, 
...
const Students: React.FC = () => {
  ...
  const canViewActions = ['system_admin', 'admin', 'principal', 'teacher'].includes(profile?.role || '');
  ...
  const handleSaveStudent = async ({ formData, imageFile }: {
    ...
    try {
      ...
      if (formData.id) {
        const { data, error } = await supabase.from('students').update(studentToSave).eq('id', formData.id).select('*, schools(name)').single();
        if (error) throw new Error(error.message);
        ...
        const { data, error } = await supabase.from('students').insert(studentToSave).select('*, schools(name)').single();
        if (error) throw new Error(error.message);
  ...
  const handleExport = () => {
    const headers = ["ID", "Name", "Email", "Class", "Section", "Roll Number", "Phone", "Status", "Shift", "School"];
    const csvContent = "data:text/csv;charset=utf-8,"
      + headers.join(",") + "\n"
      + students.map(s => `"${s.id}","${s.name}","${s.email}","${s.class}","${s.section}","${s.rollNumber}","${s.phone}","${s.status}","${s.shift || 'Morning'}","${s.school_name || ''}"`).join("\n");
      ...
      <div className="bg-white rounded-xl shadow-card border border-border overflow-hidden">
          ...
          <table className="min-w-full divide-y divide-border">
            <thead className="bg-slate-50">
              <tr>
                ...
                {canViewActions && <th className="px-6 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">Actions</th>}
              </tr>
            </thead>
        ...
        {filteredStudents.length === 0 && (
            <div className="p-12 text-center">
                ...
                {canManage && <Button onClick={handleAdd} className="mt-6" variant="secondary">Add New Student</Button>}
            </div>
        )}
      </div>
