...
-- These run as the database owner (SECURITY DEFINER) to safely check permissions
...
-- 2. Reset Profiles RLS Policies (Aggressive Cleanup)
-- We drop ALL known variations of policies to ensure a clean slate
...
-- Insert: You can insert your own profile (triggered on signup)
CREATE POLICY "profiles_insert_policy" ON profiles
FOR INSERT WITH CHECK (
  auth.uid() = id
);

-- 4. Promote User to Super Admin (Safe Method)
-- We look up the ID from auth.users since profiles might not have the email column
DO $$
DECLARE
  target_user_id UUID;
BEGIN
  -- Find the user ID from the auth system
  SELECT id INTO target_user_id FROM auth.users WHERE email = 'maamefare7419@gmail.com';
  
  IF target_user_id IS NOT NULL THEN
    -- Ensure the role column allows 'system_admin'
